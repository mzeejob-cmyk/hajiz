  ...
  return <div className="home-search" aria-label="البحث عن خدمات السفر">
    ...
    <form className="search-form" onSubmit={submit}>
      ...
      {(isFlight || isHotel) ? <div className="search-fields">
        <label className="field-origin"><span>{isHotel ? "الوجهة" : "من"}</span><input value={fields.from.label} onChange={event => update("from", createCustomLocation(event.target.value))} aria-label={isHotel ? "وجهة الفندق" : "مدينة المغادرة"} /></label>
        <button className="swap-control" type="button" onClick={swap} aria-label="تبديل نقطة المغادرة والوصول">⇄</button>
        <label className="field-destination"><span>{isHotel ? "المدينة أو الفندق" : "إلى"}</span><input value={fields.to.label} onChange={event => update("to", createCustomLocation(event.target.value))} aria-label={isHotel ? "المدينة أو الفندق" : "مدينة الوصول"} /></label>

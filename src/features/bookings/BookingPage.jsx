...
export default function BookingPage({ dataSource = myTripsDataSource }) {
  ...
  const [tickets, setTickets] = useState({ status: "idle", rows: [] })
  useEffect(() => {
  ...
  const loadTickets = () => { setTickets({ status: "loading", rows: [] }); dataSource.loadTicketDetails(reference).then(rows => setTickets({ status: "success", rows })).catch(() => setTickets({ status: "error", rows: [] })) }
  return <main data-booking-detail="owner-scoped-read">
    ...
    {booking.amount && <p><bdi dir="ltr">{booking.amount} {booking.currency}</bdi></p>}<p>طريقة الدفع: <bdi>{booking.paymentMethod}</bdi></p><p>{booking.ticketingLabel}</p>
    ...
    {booking.canViewTicketDetails && tickets.status === "idle" && <button type="button" onClick={loadTickets}>عرض بيانات التذكرة</button>}
    {tickets.status === "loading" && <p role="status">جارٍ تحميل بيانات التذكرة…</p>}{tickets.status === "error" && <p role="alert">تعذر تحميل بيانات التذكرة.</p>}
    {tickets.status === "success" && <ul>{tickets.rows.map(ticket => <li key={ticket.ticketNumber}><bdi>{ticket.ticketNumber}</bdi> · {ticket.issuedAt} · {ticket.artifactAvailability}</li>)}</ul>}
  </main>
}

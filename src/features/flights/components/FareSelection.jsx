...
export function FareSelection({ itineraryKey, initialFareKey, onBack, onContinue }) {
  const itinerary = resolveItinerary(itineraryKey)
  const [fareKey, setFareKey] = useState(resolveFare(initialFareKey)?.key ?? "checked")
    ...
    <div className="fare-layout"><main className="fare-content">
      ...
      <section className="fare-options" aria-labelledby="fare-options-title">
        ...
        <div className="fare-option-grid">{FARE_OPTIONS.map((fare) => { const selected = fare.key === fareKey; return <label key={fare.key} className={`fare-option${selected ? " is-selected" : ""}`}>
          <input className="visually-hidden" type="radio" name="fare" value={fare.key} checked={selected} onChange={() => setFareKey(fare.key)}/>
      ...
      <button type="button" onClick={() => onContinue?.(fareKey)}><span className="desktop-only">متابعة لبيانات المسافر</span><span className="mobile-only">متابعة</span></button>

...
export function FlightReview({ itineraryKey, fareKey, draft, onBack, onMissingDraft, onPayment }) {
  ...
  const continueToPayment = () => { setFeedback("خطوة الدفع هي التالية. لم يتم إنشاء عملية دفع أو حجز حتى الآن."); onPayment?.() }
  ...
  return <div className="review-page" data-view="review" data-itinerary={itinerary.key} data-fare={fare.key}><Container>
    ...
    <aside className="review-summary" aria-label="السعر النهائي"><h2 className="desktop-only">السعر النهائي</h2><Price amount={fare.amount} currency={fare.currency}/><hr className="desktop-only"/><p className="desktop-only">العرض المحدد · مسافر واحد</p><p className="desktop-only">سيتم عرض مبلغ بنكك بالجنيه قبل التحويل.</p><button type="button" onClick={continueToPayment}><span className="desktop-only">الانتقال إلى الدفع</span><span className="mobile-only">الدفع</span></button></aside>

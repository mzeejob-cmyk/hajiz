...
export function parseFlightPaymentInitiationHttpResponseV1(body) {
  ...
  if (data.paymentMethod === "bankak") {
    ...
    if (data.nextAction !== "COMPLETE_BANKAK_TRANSFER" || !exact(data.handoff, keys) || data.handoff.type !== "BANKAK_MANUAL" || !decimal(data.handoff.amount) || data.handoff.currency !== "SDG" || !/^PAY-[0-9A-F]{12}$/.test(data.handoff.paymentReference) || typeof data.handoff.bankAccountDisplayName !== "string" || !data.handoff.bankAccountDisplayName || typeof data.handoff.maskedAccountNumber !== "string" || !data.handoff.maskedAccountNumber || typeof data.handoff.receiptUploadAvailable !== "boolean") throw new TypeError("invalid Bankak handoff")
  } else {

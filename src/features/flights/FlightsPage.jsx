...
export function BookingIntentPanel({ state, checkoutResult, travelerDraft, onCreate, onEdit, onAcceptPrice, onReturn }) {
  if (!state || state.status === "idle") return null
  ...
  if (state.status === "ready_for_payment") return <section className="selection-notice" role="status"><h2>جاهز لاختيار طريقة الدفع</h2><Price amount={state.result.customerPrice.amount} currency={state.result.customerPrice.currency}/><p>رقم الطلب: <span dir="ltr">{state.result.bookingIntentId}</span></p><button type="button" disabled>اختيار طريقة الدفع — في B12</button><p>لم يتم الدفع ولم يتم تأكيد الحجز بعد، ولم يتم تثبيت مقعد.</p></section>
...
export default function FlightsPage() {
  ...
  useEffect(() => { checkoutCoordinator?.cancel(); intentCoordinator?.cancel(); setCheckoutState({ status: "idle" }); setIntentState({ status: "idle" }); setTravelerDraft(null); return () => { checkoutCoordinator?.cancel(); intentCoordinator?.cancel() } }, [checkoutCoordinator, intentCoordinator, requestKey])
  ...
  const clearIntent = () => { intentCoordinator?.cancel(); setIntentState({ status: "idle" }); setTravelerDraft(null) }

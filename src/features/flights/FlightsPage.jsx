...
export function PaymentInitiationPanel({ intent, state, onInitiate, onReturn, receiptDataSource }) {
  ...
  if (state?.status === "bankak_handoff") return <section className="selection-notice selection-notice-v2" role="status"><h2>تم إنشاء طلب الدفع</h2><p>بانتظار إتمام التحويل ومراجعته من فريق المالية.</p><Price amount={state.result.handoff.amount} currency="SDG"/><p>مرجع الدفع: <span dir="ltr">{state.result.handoff.paymentReference}</span></p><p>{state.result.handoff.bankAccountDisplayName} — <span dir="ltr">{state.result.handoff.maskedAccountNumber}</span></p><p>ينتهي طلب الدفع في <time dateTime={state.result.expiresAt}>{state.result.expiresAt}</time>.</p>{state.result.handoff.receiptUploadAvailable === true ? <BankakReceiptUpload paymentId={state.result.paymentId} dataSource={receiptDataSource}/> : <p>رفع الإيصال غير متاح حاليًا. لم يتم تأكيد الدفع أو الحجز.</p>}</section>
  if (state?.status === "psp_handoff") return <section className="selection-notice selection-notice-v2" role="status"><h2>تم إنشاء طلب الدفع</h2><p>بانتظار إتمام الدفع الآمن وتأكيده عبر المسار الموثوق.</p>{state.result.handoff.redirectUrl ? <a href={state.result.handoff.redirectUrl} rel="noreferrer">متابعة إلى الدفع الآمن</a> : <p>جلسة الدفع جاهزة، لكن لا يوجد رابط بوابة خارجي متاح في هذا التشغيل.</p>} {!state.result.handoff.live && <p>هذه جلسة Sandbox/Mock وليست عملية دفع حية.</p>}<p>لم يتم تأكيد الدفع أو الحجز.</p></section>
  if (state?.status === "initiating") return <section className="selection-notice selection-notice-v2" role="status"><h2>جارٍ إنشاء طلب الدفع…</h2><p>لن تتغير حالة الدفع أو الحجز إلى مؤكدة من هذه الخطوة.</p><button type="button" disabled>جارٍ المتابعة</button></section>

...
export function createCustomerFlightBookingIntentHttpHandlerV1({ service, resolveOwnerContext }) {
  ...
  return async function handleCustomerFlightBookingIntentV1(request) {
    ...
    try {
      ...
      if (failure instanceof FlightRepriceServiceError && failure.code === "PRICED_SELECTION_EXPIRED") return error(410, "CHECKOUT_SELECTION_EXPIRED", "The priced selection has expired.")
      if (failure instanceof FlightRepriceServiceError && failure.code === "REQUEST_TIMEOUT") return error(504, "REQUEST_TIMEOUT", "Booking intent revalidation timed out.")
      if (failure instanceof FlightRepriceServiceError && failure.code === "REPRICE_UNAVAILABLE") return error(503, "REPRICE_UNAVAILABLE", "Flight repricing is temporarily unavailable.")

...
export function AppShell({ children }) {
  ...
  const accountLink = session.status === "signed_in" ? <NavLink to="/account/trips">حسابي</NavLink> : session.status === "signed_out" ? <NavLink to="/login">تسجيل الدخول</NavLink> : <span aria-live="polite">الحساب</span>
  ...
  return <div className="app-shell">
    ...
    <header className="site-header">
      <Container className="header-inner">
        ...
        <div className="header-utilities"><span className="latin-text" dir="ltr">AED</span><button type="button">العربية</button>{accountLink}</div>
      ...
      <nav id="mobile-navigation" className={`mobile-nav${mobileOpen ? " mobile-nav--open" : ""}`} aria-label="التنقل عبر الهاتف" hidden={!mobileOpen}><Container>{PRIMARY_NAVIGATION.slice(1).map(item => <NavLink key={item.id} className={navClass} to={item.to} onClick={() => setMobileOpen(false)}>{item.label}</NavLink>)}{accountLink}</Container></nav>
    </header>

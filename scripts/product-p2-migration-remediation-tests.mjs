...
await test("favorites identity is absent from constraint guard", () => { const guard = sql.match(/-- Every named constraint[\s\S]*?-- Guarded indexes/)?.[0] ?? ""; assert.doesNotMatch(guard, /p2_favorites_owner_identity_unique/) })
await test("table-level keys contain no expression syntax", () => { assert.doesNotMatch(sql, /\b(?:unique|primary key|foreign key)\s*\([^\n)]*\([^\n)]*\)/i) })

...
await test("P53 migration creates a private non-forced-RLS initiation ledger", () => { assert.match(migration, /create table app_private\.flight_payment_initiations/); assert.match(migration, /enable row level security/); assert.match(migration, /no force row level security/); assert.doesNotMatch(migration, /^alter table app_private\.flight_payment_initiations force row level security;/m); assert.match(migration, /to anon, authenticated\s+using \(false\)\s+with check \(false\)/) })
await test("P53A migration table guard makes exact replay safe and rejects column or constraint drift", () => { assert.match(migration, /if existing_table is null then[\s\S]*create table app_private\.flight_payment_initiations[\s\S]*else[\s\S]*non-canonical owner, kind, or signature/); assert.match(migration, /existing_column_count <> 20/); assert.match(migration, /pg_catalog\.to_regtype\(item\.type_name\)/); assert.match(migration, /non-canonical constraint count/); assert.match(migration, /elsif created_now then[\s\S]*comment on constraint/); assert.match(migration, /non-canonical signature/) })
await test("P53B migration index guard validates the exact canonical index", () => { assert.match(migration, /if existing_index is null then[\s\S]*create index flight_payment_initiations_owner_created_idx/); assert.match(migration, /index_row\.indrelid/); assert.match(migration, /index_row\.indisvalid/); assert.match(migration, /index_row\.indisready/); assert.match(migration, /\(owner_id, created_at DESC\)/); assert.match(migration, /owner_created_idx:v1/) })
await test("P53C migration policy guard validates command roles predicates and signature", () => { assert.match(migration, /from pg_catalog\.pg_policy/); assert.match(migration, /existing_command <> '\*'/); assert.match(migration, /existing_role_names is distinct from array\['anon', 'authenticated'\]/); assert.equal((migration.match(/<> 'false'/g) || []).length >= 2, true); assert.match(migration, /direct_access_denied:v1/); assert.match(migration, /non-canonical definition/) })
await test("P54 migration RPCs pin search_path, share private-table ownership, and execute only as service_role", () => { assert.equal((migration.match(/security definer/g) || []).length, 2); assert.equal((migration.match(/set search_path = ''/g) || []).length, 2); assert.equal((migration.match(/to service_role/g) || []).length, 2); assert.equal(/grant execute[\s\S]*to authenticated/.test(migration), false); assert.match(migration, /booking_intents_owner is distinct from payment_initiations_owner/); assert.match(migration, /function_owner is distinct from payment_initiations_owner/) })
await test("P55 migration atomically inserts booking before its one payment", () => { assert.match(migration, /insert into public\.bookings[\s\S]*insert into public\.payments/); assert.match(migration, /booking_id uuid not null/); assert.match(migration, /flight_payment_initiations_booking_id_key unique \(booking_id\)/); assert.match(migration, /booking_intent_id uuid not null/); assert.match(migration, /flight_payment_initiations_booking_intent_id_key[\s\S]*unique \(booking_intent_id\)/) })

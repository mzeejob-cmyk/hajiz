...
await test("product p2 authority and JWT settings remain unchanged", async () => { for (const file of ["src/server/product/productP2Service.js", "src/server/product/productP2Http.js"]) execFileSync("git", ["diff", "--quiet", "ee468d55bdead5f950ed720c1a58d79cf08aef7a", "--", file]); const config = await readFile(new URL("../supabase/config.toml", import.meta.url), "utf8"); assert.match(config, /\[functions\.product-p2\]\s*verify_jwt = true/); assert.match(config, /\[functions\.catalog-public\]\s*verify_jwt = false/) })

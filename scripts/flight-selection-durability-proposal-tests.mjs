...
await test("Search validUntil is typed and parse checked", () => { has(sql, /jsonb_typeof\(v_price->'validUntil'\) <> 'string'/); has(sql, /pg_input_is_valid\(v_price->>'validUntil', 'timestamptz'::pg_catalog\.regtype\)/) })
...
await test("priced validUntil is typed and parse checked", () => { has(sql, /jsonb_typeof\(p_customer_price_snapshot->'validUntil'\) <> 'string'/); has(sql, /pg_input_is_valid\(p_customer_price_snapshot->>'validUntil', 'timestamptz'::pg_catalog\.regtype\)/) })

-- HAJIZ PSP rejected transition V1. Staging rehearsal only; not applied by this change.
-- A trusted PSP may reject only an awaiting, non-Bankak payment. Manual Bankak
-- rejection remains exclusively under the finance review command.

create or replace function app_private.enforce_payment_transition()
returns trigger language plpgsql set search_path=pg_catalog, public
as $$ begin
  if new.status is distinct from old.status and not (
    (old.status='awaiting' and new.status in ('under_review','confirmed','expired')) or
...
as $$ declare v public.payments%rowtype; begin
  ...
  if not (
    (v.method<>'bankak' and v.status='awaiting' and p_target in ('confirmed','rejected')) or

...
| `20260829120000_flight_booking_intents_v1.sql` | CODE-ONLY; NOT YET APPLIED; REPLAY-SAFE / DRIFT-DETECTING | Adds the private B11 booking-intent boundary. Canonical table, constraint, index, and policy signatures let an exact replay succeed without duplicates while rejecting incompatible same-name objects. RLS stays enabled without `FORCE RLS`; direct privileges remain revoked and service-only `SECURITY DEFINER` RPCs must share the table owner. Requires a separate Staging first-apply, exact-replay, privilege, and owner/RLS runtime gate |
| `20260829183000_flight_payment_initiation_v1.sql` | CODE-ONLY; NOT YET APPLIED; REPLAY-SAFE / DRIFT-DETECTING | Adds the B12 payment-initiation claim and atomic booking/payment materialization. Canonical table, column, constraint, index, and deny-policy signatures make exact replay safe and reject incompatible same-name objects. The Gate A-compatible definition uses non-forced RLS and shared table/function ownership without a `BYPASSRLS` dependency. Requires a separate runtime gate after B11 |
| `20260829213000_flight_supplier_booking_execution_v1.sql` | APPLIED TO HAJIZ STAGING; DO NOT REPLAY AFTER RT-01 | Adds the private B13 one-execution-per-booking record and service-only claim/result RPCs. Staging first-apply, exact replay, drift, privilege, B11/B12, and five-connection B13 claim tests passed. Runtime then exposed SQLSTATE `42702` in accepted-result persistence; the original migration is retained as applied history and is superseded for that function by the additive RT-01 correction below |
| `20260831183000_fix_flight_supplier_booking_execution_accepted_persistence.sql` | APPLIED TO HAJIZ STAGING; RT-01 ACCEPTED PATH PASS; PRODUCTION NOT APPLIED | Recorded remotely as `20260831174408`. Replaces only `public.complete_flight_supplier_booking_execution_v1` with the same signature, authority, locks, idempotency, UNKNOWN handling, and ACL while qualifying the target-table `supplier_accepted_at` self-reference. The transactional real-PostgreSQL regression in `scripts/flight-runtime-rt01-staging-test.sql` passed with `ACCEPTED`, `confirmed`, persisted supplier reference/timestamp, unchanged confirmed payment/commercial snapshot, and fixture rollback. The next runtime-gate probe stopped on a separate pre-existing `supplier_booking_ref` ambiguity in `public.record_flight_supplier_booking_failure_v1`; that follow-up is outside RT-01 |
| `20260831190000_fix_flight_supplier_booking_failure_unknown_persistence.sql` | APPLIED TO HAJIZ STAGING; RT-03 UNKNOWN PATHS PASS; PRODUCTION NOT APPLIED | Recorded remotely as `20260831180657`. Replaces only `public.record_flight_supplier_booking_failure_v1` with the same signature, authority, locks, idempotency, failure taxonomy, UNKNOWN/reconciliation behavior, and ACL while qualifying the target-table `supplier_booking_ref` self-reference. Both real-PostgreSQL UNKNOWN paths and no-blind-retry assertions in `scripts/flight-runtime-rt03-staging-test.sql` passed and all fixtures rolled back. Runtime continuation then passed B14 entry and five-session send concurrency before stopping on a separate SQLSTATE `42702` ambiguity for `issued_at` in `public.complete_flight_supplier_ticketing_v1`; that B14 defect is outside RT-03 and Production remains untouched |
| `20260830090000_flight_ticketing_confirmation_v1.sql` | APPLIED TO HAJIZ STAGING; DO NOT REPLAY AFTER RT-04 | Adds the private B14 ticketing execution and per-traveler ticket records. Staging first-apply, exact replay, drift, privilege, authority, and five-session send-concurrency checks passed. Runtime completion then exposed SQLSTATE `42702` in the `issued_at` self-reference; the original migration is retained as applied history and is superseded for that function by the additive RT-04 correction below. It never creates a booking, ticket PDF, or `completed` transition |
| `20260831193000_fix_flight_supplier_ticketing_issued_persistence.sql` | APPLIED TO HAJIZ STAGING; RT-04 RUNTIME GATE PASS; PRODUCTION NOT APPLIED | Recorded remotely as `20260901160250`. Replaces only `public.complete_flight_supplier_ticketing_v1` with the same signature, authority, locks, idempotency, UNKNOWN/reconciliation behavior, traveler coverage, evidence validation, lifecycle, and ACL while qualifying the target-table `issued_at` self-reference. Transactional real-PostgreSQL tests passed PROCESSING, trusted ISSUED persistence, exact replay, crash-window UNKNOWN/reconciliation, failure taxonomy, no-blind-reissue, evidence rejection/completeness, two-owner isolation, My Trips reads, payment/commercial firewalls, and RT-01/RT-03 regression; all synthetic fixtures rolled back |

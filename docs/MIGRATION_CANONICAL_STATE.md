...
| `20260829213000_flight_supplier_booking_execution_v1.sql` | CODE-ONLY; NOT YET APPLIED; REPLAY-SAFE / DRIFT-DETECTING | Adds the private B13 one-execution-per-booking record and service-only claim/result RPCs. Requires confirmed payment plus `payment_confirmed` booking, exact B11/B12/offer identity, one request-sent attempt, unknown-outcome blocking, and supplier acceptance before `confirmed`. Requires a separate runtime gate after B11/B12; never implies ticketing |

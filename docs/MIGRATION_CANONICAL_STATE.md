...
| `20260830090000_flight_ticketing_confirmation_v1.sql` | CODE-ONLY; NOT YET APPLIED; REPLAY-SAFE / DRIFT-DETECTING | Adds the private B14 ticketing execution and per-traveler ticket records. Requires `confirmed` booking plus exact B13 `ACCEPTED` lineage, one supplier send claim, complete returned ticket evidence before `ticketed`, owner-scoped My Trips reads, and unknown-outcome reconciliation. It never creates a booking, ticket PDF, or `completed` transition |

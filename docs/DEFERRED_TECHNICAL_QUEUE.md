...
- **B6-01 — LOW / NONBLOCKING / HASH DOMAIN TAG:** `hcg_v1_` and `hca_v1_` identify the digest externally, but the version/domain tag is not part of the hashed input. No collision defect is known. Include the domain tag inside the hash input, following the itinerary fingerprint precedent, when convenient with B6-02 and before endpoint exposure if version-compatible.
- **B6-02 — REQUIRED IN HTTP ENDPOINT BATCH BEFORE PUBLIC SELECTION HANDLE GO-LIVE / CANONICAL SERIALIZATION:** customer alternative IDs currently hash `JSON.stringify` output from literal objects, which is deterministic for the current construction but can change with object-key reordering. Adopt reviewed key-sorted canonical serialization before these IDs become public selection handles, and do not casually change the public ID contract after endpoint exposure.
- **B6-03 — BEFORE POLISHED CUSTOMER RESULTS / LOW / GROUP ITINERARY SOURCE:** display itinerary currently comes from `alternatives[0].offer`, even when that alternative is later excluded publicly. Fingerprinted core itinerary fields remain equivalent, but non-fingerprinted display fields such as `marketingCarrierName` may differ. Later derive display itinerary from the first retained customer-visible alternative or independently normalize display fields.

...
| MS-09 | P1 | Partial — HTTP customer search endpoint complete, frontend not wired | `customer-flight-search-http/v1` validates the exact public request, invokes the full trusted application pipeline under a global budget, and emits only `customer-flight-search/v1`; host deployment and frontend reconciliation remain absent | Public Search Boundary |
